...
namespace ManagedIdentitySample.Client.Controllers
{
    ...
    public class Controller : ControllerBase
    {
        ...
        {
            var content = $"success {_appSettings.Message}";
            Console.WriteLine(content);
            return Ok(content);
        }

        [HttpGet]
        [Route("~/workingrevison")]
        public async Task<IActionResult> GetWorkingRevison()
        {
            try
            {
                string revison = Environment.GetEnvironmentVariable("CONTAINER_APP_REVISION");
                ...
                string url = $"http://localhost:{daprPort}/v1.0/invoke/{targetDaprAppId}/method/workingrevison";

                Console.WriteLine($"[Client] GetWorkingRevison url: {url}");
                ...
                Console.WriteLine($"[Client] Request to get working revison has been sent, the status code is {response.StatusCode}, content is {content}");
                return Ok($"Frontend revision {revison}. Backend revison {content}");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[Client] GetWorkingRevison Exception:{ex.Message}");
        ...
        public async Task<IActionResult> SetStateWithoutDapr(string usetoken, [FromBody] Dictionary<string, string> payload)
        {
            try
            {
                ...
                string targetUri = $"https://{targetAppName}.internal.{envDNSSuffix}/setstate";
        ...
        [HttpPost]
        [Route("~/writestoragevolume")]
        public IActionResult WriteStorageVolume(string fileName, string mountPath, string content)
        {
            try
            {
                string path = $"{mountPath}/{fileName}";
            ...
            {
                Console.WriteLine($"[Client] WriteStorageVolume Exception: {ex}");
                return BadRequest($"[Client] WriteStorageVolume Exception {ex}");
            }
        }
